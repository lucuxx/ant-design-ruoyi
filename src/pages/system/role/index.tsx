...
import { FooterToolbar, GridContent } from '@ant-design/pro-components';
...
import PermissionsForm from './components/permission';
import { getDict } from '../dict/service';
import type { DataNode } from 'antd/lib/tree';
import { getMenuTree } from '../menu/service';
import { formatTreeSelectData } from '@/utils/utils';
import { getDeptTreeRole } from '@/pages/system/dept/service';
import { getRole } from '@/pages/system/role/service';
...
const handleUpdateDataScope = async (fields: RoleType) => {
  const hide = message.loading('正在配置');
  try {
    const resp = await updateRoleDataScope(fields);
    hide();
    if (resp.code === 200) {
      message.success('配置成功');
    } else {
    ...
    },
    {
      ...
      render: (_, record) => [
        ...
        <Button
          ...
          key="permissions"
          onClick={() => {
            getDeptTreeRole(record.roleId).then((res: any) => {
              if (res.code === 200) {
                const treedata = formatTreeSelectData(res.depts);
                setDeptTree(treedata);
                const newKeys = res.checkedKeys.map((i: any) => {
                  return String(i);
                });
                setDeptIds(newKeys);
  ...
  return (
    <GridContent>
      ...
      <PermissionsForm
        onSubmit={async (values) => {
          let success = false;
          success = await handleUpdateDataScope({ ...values } as RoleType);
          if (success) {
